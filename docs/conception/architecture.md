...
docs/            cette documentation (Markdown) ; docs/_theme/ : gabarit, styles, sommaire
...
- `biomes.ts` : les **vingt îles** (nom, matière, classe, module, bloc, créature et ses phrases, Gardien et ses répliques, quêtes) et les **blocs**.
...
- `engine.ts` : logique pure et testée du jeu : score, étoiles, blocs, XP, répétition espacée, série de régularité et coffres, adaptation du niveau, inventaire.
...
- `world/` : le monde. `map.ts` place les îles (cœur de 16 × 16, terre irrégulière, altitude par classe, relief, paysage) ; `archipelago.ts` définit les ouvrages, leurs coûts et conditions, et calcule ce qui est ouvert ; `plans.ts` et `plans/*.json` décrivent les bâtiments à reconstruire ; `terrain.ts` et `mesher.ts` produisent les cubes ; `daylight.ts` le cycle jour-nuit ; `goals.ts` le prochain objectif et les explications d’île fermée.
- `three/` : le rendu Three.js (`WorldCanvas.tsx`, textures pixel générées par le code, détection WebGL). `WorldPage.tsx` est le monde plein écran ; `IslandSheet.tsx` le panneau d’île ; `BloclandPage.tsx` et `BiomePage.tsx` la vue simple.
