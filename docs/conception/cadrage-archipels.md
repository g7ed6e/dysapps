...
3. ✅ **Le Bloc-Navire au quai** : le navire sort du terrain et devient un objet animé (`vehiclePlacement` dans `terrain.ts`, groupe à part dans `WorldCanvas`) : il tangue sur l'eau, plane à hauteur de quai dans le ciel, son ballon se balance au sommet du mât ; ses cases fantômes se posent en les touchant, le toucher ailleurs ouvre le panneau du port sur la section Bloc-Navire (mise en avant) ; la flèche jaune flotte sur le chantier quand le panneau du port est ouvert et qu'il reste des cases à poser ; le navire est dessiné dans l'écran de voyage depuis la PR 1.
4. ✅ **Le voyage** : la cinématique en deux temps (`world/voyage.ts`) : le bonhomme marche jusqu'au pont, le navire s'éloigne (vue de côté, la caméra s'écarte ; écume à la voile, flamme au réacteur), le voile, le changement d'archipel (la scène est reconstruite), le navire accoste et le bonhomme débarque ; huit secondes, cinq pour un retour ; les sons (corne de brume, voile, brûleur, réacteur, carillon d'arrivée) ; un toucher, Entrée, Espace, Échap ou le bouton « Arriver » font arriver tout de suite ; sous « Réduire les animations », l'écran fixe de la PR 1.
5. ✅ **Les quatre archipels et l'accueil** : la page « Les quatre archipels » (`#/aventure/monde`, un panneau HTML : où l'on est, îles ouvertes et Gardiens vaincus par archipel, ce qu'il faut pour aller plus loin, le navire dessiné, boutons « Aller au port » et « Voir le chantier ») ; les deux bulles de première arrivée dans chaque archipel (`arrivals.ts`, en 3D et en vue simple sur le port) ; l'écran du Gardien qui dit quand le kit du navire arrive, avec « Aller au port » ; l'écran de récompense qui dit « Le Bloc-Navire a tous ses blocs ! » ; l'accueil qui annonce « Le Bloc-Navire est prêt ! ».

## 11. À venir

Les cinq PR sont livrées. Rien de décidé au-delà : jouer, écouter les retours des enfants sur le premier voyage (le nombre de Gardiens exigés et la taille du premier chantier se règlent dans `vehicle.ts`).
