...
it('le prochain objectif dit ce qu’il manque pour le plan, ou pour l’ouvrage le moins cher', () => {
  ...
  expect(nextGoal(fresh, 'foret')).toBe('Encore 16 bois pour La cabane de Mousso, ou 3 blocs pour le pont vers Mine des lettres.');
  const some = sanitizeState({ inventory: { bois: 5 } });
  expect(nextGoal(some, 'foret')).toBe('Encore 11 bois pour La cabane de Mousso, ou tu peux construire le pont vers Mine des lettres.');
  const rich = sanitizeState({ inventory: { bois: 20 } });
  expect(nextGoal(rich, 'foret')).toBe('Tu as tout pour finir La cabane de Mousso : pose tes blocs, ou tu peux construire le pont vers Mine des lettres.');
